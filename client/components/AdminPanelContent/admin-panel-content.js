              ...
              <main class="modal-create-client-main">
                <form @submit.prevent="createClient(); toggleModalCreateClient();">
                  ...
                  <div class="create-client-grid">
                    ...
                    <select name="marital_status" v-model="client_data.marital_status" required>
  ...
  methods: {
    ...
    async searchClient() {
      try {
        ...
        const isInputNotEmpty = search_value !== "";

        if (isInputNotEmpty) {
          const SEARCH_CLIENT_API = `${app.$data.BASE_URL}/api/clients/search/${search_value}`;

          const response = await axios.get(SEARCH_CLIENT_API);
          this.search_data = response.data;

          const isSearchDataEmpty = this.search_data.length === 0;

          if (isSearchDataEmpty) {
            this.message = "Nenhum resultado foi encontrado!";
            this.hasError = true;
